...
class Softmax_Norm_Layer(Layer):
    ...
    def __init__(
        ...
        self.compute_available_time = 0  # the cycle at which the adders, inverse and multiply units are idle for new computation
    ...
    def compute(self):
        '''
            Normalization
        '''
        num_sum_stages = int(math.log2(next_power_of_2(self.N)))
        latency = num_sum_stages * C.SUM_LAT
        latency += C.ADC_LAT
        latency += C.MUL_LAT

        num_adds = sum(2**i for i in range(num_sum_stages))
        self.energy_stats["sum_energy"] += num_adds * C.SUM_ENERGY
        self.energy_stats["act_energy"] += C.ACT_ENERGY
        self.energy_stats["mul_energy"] += C.MUL_ENERGY * self.N
