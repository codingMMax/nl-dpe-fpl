...
def _gen_dimm_score_matrix(n_seq: int, d_head: int, h_dimm: int,
    ...
    lines.append(f"    // BACK_TO_BACK_MODE=1 lets the FSM return from S_OUTPUT to")
    lines.append(f"    // S_LOAD_Q on a `next_q_row_trigger` pulse, enabling")
    ...
    lines.append(f"    // AH Step B-2 (additive): pulses high for one cycle when the")
    ...
    lines.append(f"                end")
    lines.append(f"                S_OUTPUT: begin")
    lines.append(f"                    if (ready_n) score_read_addr <= score_read_addr + 1;")
    lines.append(f"                    // AH Step B-2 (additive): back-to-back Q-row recycle.")
    lines.append(f"                    // When mode=1 and head FSM pulses next_q_row_trigger,")
    lines.append(f"                    // re-enter S_LOAD_Q to absorb the next Q row and recompute")
    lines.append(f"                    // scores. K SRAM persists; we skip S_LOAD_K. Default mode=0")
    lines.append(f"                    // makes this branch unreachable, preserving current behaviour.")
    lines.append(f"                    if (BACK_TO_BACK_MODE && next_q_row_trigger) begin")
    lines.append(f"                        state <= S_LOAD_Q;")
    lines.append(f"                        q_write_addr <= 0;")
    lines.append(f"                        q_read_addr <= 0;")
    lines.append(f"                        score_write_addr <= 0;")
    lines.append(f"                        score_read_addr <= 0;")
    lines.append(f"                        mac_count <= 0;")
    lines.append(f"                        score_idx <= LANE_IDX * {stride};")
    lines.append(f"                        acc_clear <= 1;")
    if dual_identity:
        lines.append(f"                        feed_half <= 0;")
        lines.append(f"                        feed_phase <= 0;")
...
def _gen_softmax_dpe(n_seq: int, d_head: int, h_dimm: int,
    ...
    lines.append(f"    parameter WIDE_ADDR_MODE = 0,")
    lines.append(f"    // AH Step B-5 (additive, default off): BACK_TO_BACK_MODE=1 lets the")
    lines.append(f"    // softmax FSM return from SM_OUTPUT to SM_IDLE on a `next_q_row_trigger`")
    lines.append(f"    // pulse, enabling back-to-back softmax across Q rows. Default 0 keeps the")
    lines.append(f"    // canonical SM_OUTPUT-as-terminal behaviour exactly.")
    lines.append(f"    parameter BACK_TO_BACK_MODE = 0")
    lines.append(f")(")
    lines.append(f"    input wire clk, input wire rst,")
    ...
    lines.append(f"    // AH Step B-4 (additive): Q-row counter for wide-address mode.")
    lines.append(f"    input wire [7:0] q_row_idx_i,")
    lines.append(f"    // AH Step B-5 (additive): pulses high one cycle when the head FSM wants")
    lines.append(f"    // softmax to begin processing the next Q row. Tied to 1'b0 by callers")
    lines.append(f"    // running in single-Q-row mode; ignored when BACK_TO_BACK_MODE=0.")
    lines.append(f"    input wire next_q_row_trigger,")
    lines.append(f"    // AH Step B-5 (additive): pulses high for one cycle when the softmax")
    lines.append(f"    // FSM enters SM_OUTPUT (= S_NORM completion = data ready in out_sram).")
    lines.append(f"    output wire softmax_done,")
    lines.append(f"    output wire [DATA_WIDTH-1:0] data_out,")
    ...
    lines.append(f"                    if (!ready_n) out_read_addr <= out_read_addr + 1;")
    lines.append(f"                    // AH Step B-5 (additive): back-to-back Q-row recycle.")
    lines.append(f"                    // When mode=1 and head pulses next_q_row_trigger, SM_OUTPUT")
    lines.append(f"                    // returns to SM_IDLE so SM_LOAD can absorb the next Q row's")
    lines.append(f"                    // scores. Default mode=0 keeps SM_OUTPUT terminal.")
    lines.append(f"                    if (BACK_TO_BACK_MODE && next_q_row_trigger) begin")
    lines.append(f"                        sm_state <= SM_IDLE;")
    lines.append(f"                        sm_count <= 0;")
    lines.append(f"                        in_write_addr <= 0;")
    lines.append(f"                        in_read_addr <= 0;")
    lines.append(f"                        out_read_addr <= 0;")
    lines.append(f"                        // out_write_addr stays where it left off (wide-mode")
    lines.append(f"                        // SRAM accumulates across Q rows); in_write_addr resets")
    lines.append(f"                        // to 0 so SM_LOAD can write next Q row's scores starting")
    lines.append(f"                        // at the beginning of in_sram (single-Q-row scope per")
    lines.append(f"                        // SM_LOAD; wide_addr_mode is on out_sram).")
    lines.append(f"                    end")
    lines.append(f"                end")
    lines.append(f"            endcase")
    lines.append(f"        end")
    ...
    lines.append(f"    // AH Step B-5: per-Q-row softmax-done pulse.")
    lines.append(f"    // High for one cycle on the entry edge into SM_OUTPUT (= SM_NORMALIZE")
    lines.append(f"    // exit, sm_count==N_PER_LANE+1). Used by the head FSM to count completed")
    lines.append(f"    // softmax rows. Independent of BACK_TO_BACK_MODE — always observable.")
    lines.append(f"    reg sm_state_was_normalize;")
    lines.append(f"    always @(posedge clk or posedge rst) begin")
    lines.append(f"        if (rst) sm_state_was_normalize <= 0;")
    lines.append(f"        else sm_state_was_normalize <= (sm_state == SM_NORMALIZE);")
    lines.append(f"    end")
    lines.append(f"    assign softmax_done = sm_state_was_normalize && (sm_state == SM_OUTPUT);")
    lines.append(f"endmodule")
    return "\n".join(lines)
...
def _gen_dimm_weighted_sum(n_seq: int, d_head: int, h_dimm: int,
    ...
    lines.append(f"    // AH cycle alignment Step B-3 (additive, default off):")
    lines.append(f"    // BACK_TO_BACK_MODE=1 lets the wsum FSM return from WS_OUTPUT")
    ...
    lines.append(f"    // AH Step B-3 (additive): pulses high one cycle when head FSM")
    ...
    lines.append(f"    output wire valid_n,")
    lines.append(f"    // AH Step B-5 (additive): per-Q-row wsum-done pulse. Asserts for one")
    lines.append(f"    // cycle on the entry edge into WS_OUTPUT (= last column written, FSM")
    lines.append(f"    // about to terminate single-Q-row scope). Used by the head FSM to")
    lines.append(f"    // count completed wsum rows in S_PHASE3.")
    lines.append(f"    output wire wsum_done")
    lines.append(f");")
    lines.append(f"")
    ...
    lines.append(f"                    ws_state <= WS_EXP_FEED;")
    lines.append(f"                end")
    lines.append(f"                WS_OUTPUT: begin")
    lines.append(f"                    if (ready_n) out_read_addr <= out_read_addr + 1;")
    lines.append(f"                    // AH Step B-3 (additive): back-to-back Q-row recycle.")
    lines.append(f"                    // When mode=1 and head FSM pulses next_q_row_trigger,")
    lines.append(f"                    // re-enter WS_LOAD_A to absorb the next Q row's softmax")
    lines.append(f"                    // weights. v_sram persists; WS_LOAD_V is skipped.")
    lines.append(f"                    // Default mode=0 makes this branch unreachable.")
    lines.append(f"                    if (BACK_TO_BACK_MODE && next_q_row_trigger) begin")
    lines.append(f"                        ws_state <= WS_LOAD_A;")
    lines.append(f"                        attn_write_addr <= 0;")
    lines.append(f"                        attn_read_addr <= 0;")
    lines.append(f"                        out_write_addr <= 0;")
    lines.append(f"                        out_read_addr <= 0;")
    lines.append(f"                        ws_feed_count <= 0;")
    lines.append(f"                        ws_m <= LANE_IDX * M_PER_LANE;")
    lines.append(f"                        ws_acc_clear <= 1;")
    ...
    lines.append(f"    // AH Step B-5: per-Q-row wsum-done pulse.")
    lines.append(f"    // High for one cycle on the entry edge into WS_OUTPUT (= WS_EXP_DRAIN")
    lines.append(f"    // exit when ws_m hit last column). Used by the head FSM to count")
    lines.append(f"    // completed wsum rows in S_PHASE3.")
    lines.append(f"    reg ws_state_was_exp_drain;")
    lines.append(f"    always @(posedge clk or posedge rst) begin")
    lines.append(f"        if (rst) ws_state_was_exp_drain <= 0;")
    lines.append(f"        else ws_state_was_exp_drain <= (ws_state == WS_EXP_DRAIN);")
    lines.append(f"    end")
    lines.append(f"    assign wsum_done = ws_state_was_exp_drain && (ws_state == WS_OUTPUT);")
    lines.append(f"endmodule")
    return "\n".join(lines)
...
def _gen_attention_top(v: int, h: int, n_seq: int, d_head: int,
    ...
    lines.append(f"        .data_in(data_out_score),.q_row_idx_i(8'd0),.next_q_row_trigger(1'b0),")
    lines.append(f"        .softmax_done(),")
    ...
    lines.append(f"        .next_q_row_trigger(1'b0),.q_row_idx_i(8'd0),")
    lines.append(f"        .wsum_done(),")
