...
def render_top_module(out_path: Path) -> None:
    ...
    rtl.append("    // AH Step 1: per-cycle DIMM score-stage valid (unused at head level")
    rtl.append("    // for now; Step 4 will consume it to wire score → softmax streaming).")
    rtl.append("    wire dimm_score_valid_o;  /* unused — Step 1 observation port */")
    rtl.append("    wire dimm_rst_internal = rst | dimm_soft_rst;")
    rtl.append("")
    rtl.append("    // AH B-2/B-3/B-4 (additive, default off): back-to-back triggers and")
    rtl.append("    // wide-address mode tied off here; B-5 rewires from 3-phase head FSM.")
    rtl.append("    wire [7:0] dimm_q_row_idx = 8'd0;  // B-5 will drive from counter")
    rtl.append("")
    rtl.append("    nldpe_dimm_top #(")
    rtl.append("        .N(N_SEQ), .D(D_HEAD), .W(W), .DATA_WIDTH(DATA_WIDTH),")
    rtl.append("        .SCORE_BACK_TO_BACK_MODE(0),  // B-5 will override")
    rtl.append("        .WSUM_BACK_TO_BACK_MODE(0),   // B-5 will override")
    rtl.append("        .SCORE_WIDE_ADDR_MODE(0),     // B-5 will override")
    rtl.append("        .SOFTMAX_WIDE_ADDR_MODE(0),   // B-5 will override")
    rtl.append("        .WSUM_WIDE_ADDR_MODE(0)       // B-5 will override")
    ...
    rtl.append("        .score_next_q_row_trigger_i(1'b0),  // B-5 will override")
    rtl.append("        .wsum_next_q_row_trigger_i(1'b0),   // B-5 will override")
    rtl.append("        .q_row_idx_i(dimm_q_row_idx)        // B-5 will drive from counter")
    rtl.append("    );")
    rtl.append("")
