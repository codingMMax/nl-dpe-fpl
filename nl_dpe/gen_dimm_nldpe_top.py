...
def _gen_dimm_top_nldpe(n_seq, d_head, crossbar_cols, W, data_width=40):
    ...
    L.append(f"        // LANE_IDX + W parameters make this lane handle keys {{2*lane, 2*lane+1,")
    L.append(f"        // 2*lane+2W, 2*lane+2W+1, ...}}, giving N/W=8 scores per lane at N=128, W=16.")
    L.append(f"        dimm_score_matrix #(.N(N), .d(D), .DATA_WIDTH(DATA_WIDTH),")
    L.append(f"                            .LANE_IDX(lane), .W(W),")
    L.append(f"                            .BACK_TO_BACK_MODE(SCORE_BACK_TO_BACK_MODE)) score_inst (")
    ...
    L.append(f"")
    L.append(f"        // Stage 2: softmax_approx (normalize) — per-lane over N/W=8 elements.")
    L.append(f"        // NOTE: sum is LOCAL (8 elements), not global. A cross-lane reduction")
    L.append(f"        // would be needed for mathematically correct softmax output. For Phase")
    L.append(f"        // J cycle alignment we accept local-only softmax; wsum outputs X here.")
    L.append(f"        softmax_approx #(.N(N), .d(D), .DATA_WIDTH(DATA_WIDTH),")
    L.append(f"                         .LANE_IDX(lane), .W(W)) softmax_inst (")
    L.append(f"            .clk(clk), .rst(rst),")
    ...
    L.append(f"        // LANE_IDX + W make this lane handle output cols")
    L.append(f"        //   [LANE_IDX * d/W, (LANE_IDX+1) * d/W), giving d/W=4 cols per lane at d=64, W=16.")
    L.append(f"        dimm_weighted_sum #(.N(N), .d(D), .DATA_WIDTH(DATA_WIDTH),")
    L.append(f"                            .LANE_IDX(lane), .W(W),")
    L.append(f"                            .BACK_TO_BACK_MODE(WSUM_BACK_TO_BACK_MODE)) wsum_inst (")
    L.append(f"            .clk(clk), .rst(rst),")
