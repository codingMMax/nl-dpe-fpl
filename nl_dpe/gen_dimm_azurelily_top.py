...
def _gen_dimm_top_azurelily(n_seq, d_head, crossbar_cols, W, data_width=40):
    ...
    L.append(f"                    state <= S_WAIT_QK; mac_count <= 0;")
    L.append(f"                end")
    L.append(f"            end")
    L.append(f"            S_WAIT_QK: if (|lane_valid) state <= S_OUTPUT;  // simplified")
    ...
    L.append(f"            .valid(state == 3'd2 /*S_FEED_QK*/),")
    L.append(f"            .ready_n(1'b0),")
    L.append(f"            .data_a(q_sram_out),")
    L.append(f"            .data_b(k_sram_out ^ {dw}'d{1}),  // anti-merge")
