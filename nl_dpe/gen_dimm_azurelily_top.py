...
def _gen_dimm_top_azurelily(n_seq, d_head, crossbar_cols, W, data_width=40):
    ...
    L.append(f"    // SCORE_BACK_TO_BACK_MODE — when set, lets the top-level FSM")
    L.append(f"    // recycle from S_WAIT_QK back to S_FEED_QK on a `score_next_q_row_trigger_i`")
    L.append(f"    // pulse, enabling back-to-back mac_qk firing across Q rows.")
    L.append(f"    // WSUM_BACK_TO_BACK_MODE — when set, lets the FSM recycle from")
    L.append(f"    // S_OUTPUT back to S_FEED_QK on a `wsum_next_q_row_trigger_i` pulse,")
    L.append(f"    // re-arming the dsp_mac auto-recycle for the next Q row's mac_sv.")
    ...
    L.append(f"    // AH Step B-5 (additive, default off): SOFTMAX_BACK_TO_BACK_MODE is")
    L.append(f"    // a no-op for AL (clb_softmax already auto-recycles S_NORM → S_LOAD).")
    L.append(f"    // Kept for symmetry with NL DIMM-top so the head FSM can drive the")
    L.append(f"    // same parameter set on both architectures.")
    L.append(f"    parameter SOFTMAX_BACK_TO_BACK_MODE = 0,")
    L.append(f"    // SCORE_WIDE_ADDR_MODE — no-op on AL (mac_qk doesn't need wide-addr;")
    L.append(f"    // softmax's sm_buf is the only Q-row-aware SRAM in the AL DIMM-top).")
    ...
    L.append(f"    // lane's mac_qk dsp_mac fires (dsp_mac.valid_n). Step 4 will")
    L.append(f"    // consume this to stream score → softmax. Today unused at top.")
    L.append(f"    output wire score_valid_o,")
    L.append(f"    // AH Step B-5 (additive): AND-reduced score-stage done — asserts only")
    L.append(f"    // when ALL W lanes' mac_qk simultaneously fire valid_n. Cosmetic on AL")
    L.append(f"    // (single shared FSM means lanes always lockstep) but kept for parity")
    L.append(f"    // with NL DIMM-top.")
    L.append(f"    output wire score_all_done_o,")
    L.append(f"    // AH Step B-5 (additive): per-Q-row softmax-done pulse, AND-reduced")
    L.append(f"    // across W lanes. Asserts when ALL W clb_softmax instances enter S_NORM")
    L.append(f"    // for the current Q row (= scores fully captured, normalisation begins).")
    L.append(f"    output wire softmax_done_o,")
    L.append(f"    // AH Step B-5 (additive): per-Q-row wsum-done pulse — OR-reduced across")
    L.append(f"    // W lanes' mac_sv valid_n.  Fires when the FIRST lane's mac_sv produces")
    L.append(f"    // its first valid_n for this Q row. Used by the head FSM as the row-")
    L.append(f"    // advance trigger; matches sim's gemm_dsp(fires_per_lane_per_row=1) FSM-")
    L.append(f"    // cutoff semantics.")
    L.append(f"    output wire wsum_done_o,")
    L.append(f"    // AH cycle alignment Steps B-2/B-3 (additive, default off):")
    L.append(f"    // back-to-back Q-row trigger pulses for the score and wsum stages.")
    L.append(f"    // When the corresponding mode parameter is 1 (B-5 territory), a")
    ...
    L.append(f"    // AH Step B-5 (additive): softmax trigger.  No-op on AL (clb_softmax")
    L.append(f"    // auto-recycles); kept for symmetry with NL DIMM-top interface.")
    ...
    L.append(f"    wire [W-1:0] lane_score_valid;     // AH Step 1: per-lane mac_qk valid_n")
    L.append(f"    wire [W-1:0] lane_softmax_done;    // AH Step B-5: per-lane S_LOAD→S_INV edge")
    ...
    L.append(f"    // AH B-5 finish: K/V amortization.")
    L.append(f"    // ALL counters and FSM state are owned by ONE always block (matches NL's")
    L.append(f"    // pattern, avoids multi-driver issues on q_w_addr).  K/V counters saturate")
    L.append(f"    // at thresholds so S_LOAD condition stays stable across Q-row recycling.")
    L.append(f"")
    ...
    L.append(f"    // AH B-5 finish: gate q_sram w_en by state==S_LOAD/S_IDLE/S_RELOAD_Q so")
    L.append(f"    // writes occur only during the legacy initial load and the K/V-amortized")
    L.append(f"    // per-Q-row reload.  Additionally accept writes during S_WAIT_QK/S_OUTPUT")
    L.append(f"    // when the back-to-back trigger fires — this captures the head's first")
    L.append(f"    // valid_q packed word on the same cycle as the trigger pulse (the FSM")
    L.append(f"    // transitions to S_RELOAD_Q on the next clock edge so the gate's S_RELOAD_Q")
    L.append(f"    // path takes over for cycles 2..12).  When trigger_pulse is high, q_w_addr_eff")
    L.append(f"    // = 0 (overrides the saturated q_w_addr=13) so mem[0] is written.  K/V SRAM")
    L.append(f"    // writes are ungated (one-shot during initial load).")
    L.append(f"    wire trigger_pulse = (SCORE_BACK_TO_BACK_MODE && score_next_q_row_trigger_i) ||")
    L.append(f"                         (WSUM_BACK_TO_BACK_MODE && wsum_next_q_row_trigger_i);")
    L.append(f"    // AH B-5 finish: alias for clarity; trigger_pulse is the head's signal that")
    L.append(f"    // it's pushing a new Q row.  Wsum_done_o is wired to fire at S_FEED_QK exit")
    L.append(f"    // so the head sees it after DIMM is ready for the next Q row.")
    L.append(f"    wire pending_advance_in_b2b = trigger_pulse;")
    L.append(f"    // q_sram_w_en: accept writes during S_LOAD/S_IDLE/S_RELOAD_Q.  Also accept")
    L.append(f"    // during S_WAIT_QK/S_OUTPUT when pending_advance is high and the trigger has")
    L.append(f"    // landed (the FSM transitions to S_RELOAD_Q on the same edge — this captures")
    L.append(f"    // the head's first valid_q packed word for the new Q row).")
    L.append(f"    wire q_sram_w_en = valid_q && (state == S_IDLE || state == S_LOAD || state == S_RELOAD_Q ||")
    L.append(f"                                   ((state == S_WAIT_QK || state == S_OUTPUT) && pending_advance_in_b2b));")
    L.append(f"    // q_w_addr_eff: substitute 0 when the latched advance fires while we're still")
    L.append(f"    // in S_WAIT_QK/S_OUTPUT (the FSM hasn't yet transitioned to S_RELOAD_Q).")
    L.append(f"    wire [{addr_w_shared}-1:0] q_w_addr_eff =")
    L.append(f"        ((state == S_WAIT_QK || state == S_OUTPUT) && pending_advance_in_b2b) ? {addr_w_shared}'d0")
    L.append(f"                                                                                 : q_w_addr;")
    L.append(f"    sram #(.N_CHANNELS(1),.DATA_WIDTH(DATA_WIDTH),.DEPTH({depth_q}))")
    L.append(f"        q_sram (.clk(clk),.rst(rst),.w_en(q_sram_w_en),.r_addr(q_r_addr),")
    L.append(f"                .w_addr(q_w_addr_eff),.sram_data_in(data_in_q),.sram_data_out(q_sram_out));")
    ...
    L.append(f"    // FSM: LOAD → FEED_QK → WAIT_QK → S_OUTPUT (with S_RELOAD_Q for K/V amortization)")
    L.append(f"    localparam S_IDLE=3'd0, S_LOAD=3'd1, S_FEED_QK=3'd2, S_WAIT_QK=3'd3,")
    L.append(f"               S_RELOAD_Q=3'd4, S_FEED_SV=3'd5, S_OUTPUT=3'd6;")
    ...
    L.append(f"                       q_r_addr <= 0; k_r_addr <= 0; v_r_addr <= 0;")
    L.append(f"                       q_w_addr <= 0; k_w_addr <= 0; v_w_addr <= 0; end")
    L.append(f"        else begin")
    L.append(f"            // K/V counters: monotonic increment, saturate at threshold so the")
    L.append(f"            // S_LOAD condition stays stable across recycling (K/V amortization).")
    L.append(f"            if (valid_k && k_w_addr < {load_threshold_k}) k_w_addr <= k_w_addr + 1;")
    L.append(f"            if (valid_v && v_w_addr < {load_threshold_v}) v_w_addr <= v_w_addr + 1;")
    L.append(f"            // q_w_addr is the per-Q-row Q SRAM write address.  Increments")
    L.append(f"            // on valid_q while in S_LOAD/S_RELOAD_Q/S_IDLE (saturates at threshold).")
    L.append(f"            // S_IDLE included so the head's first valid_q pulse (which causes")
    L.append(f"            // the S_IDLE→S_LOAD transition) increments q_w_addr 0→1 alongside.")
    L.append(f"            if ((state == S_IDLE || state == S_LOAD || state == S_RELOAD_Q) && valid_q")
    L.append(f"                && q_w_addr < {load_threshold_q})")
    L.append(f"                q_w_addr <= q_w_addr + 1;")
    L.append(f"            case (state)")
    L.append(f"            S_IDLE: if (valid_q || valid_k || valid_v) begin")
    L.append(f"                       state <= S_LOAD;")
    L.append(f"                       // q_w_addr is left as-is (0 from rst, or whatever the TB")
    L.append(f"                       // forced via hierarchical assignment).  The S_LOAD branch's")
    L.append(f"                       // generic increment will pick up valid_q on subsequent cycles.")
    L.append(f"                    end")
    ...
    L.append(f"                    end")
    L.append(f"            S_RELOAD_Q: if (q_w_addr >= {load_threshold_q}) begin")
    L.append(f"                        // Per-Q-row reload complete — re-enter S_FEED_QK.  K/V SRAM")
    L.append(f"                        // contents and addresses persist (K/V amortization).")
    L.append(f"                        state <= S_FEED_QK; mac_count <= 0;")
    L.append(f"                    end")
    ...
    L.append(f"                end")
    L.append(f"            end")
    L.append(f"            S_WAIT_QK: begin")
    L.append(f"                // AH B-5 finish (K/V amortization): on a pending advance request,")
    L.append(f"                // recycle S_WAIT_QK → S_RELOAD_Q. Q-side addresses reset; K/V SRAM")
    L.append(f"                // contents and addresses persist (K/V amortization).")
    L.append(f"                // Default mode=0 keeps the canonical S_WAIT_QK → S_OUTPUT path.")
    L.append(f"                if (pending_advance_in_b2b) begin")
    L.append(f"                    state <= S_RELOAD_Q;")
    L.append(f"                    mac_count <= 0;")
    L.append(f"                    row_count <= 0;")
    L.append(f"                    q_w_addr <= valid_q ? {addr_w_shared}'d1 : {addr_w_shared}'d0;")
    L.append(f"                    q_r_addr <= 0;")
    L.append(f"                end else if (|lane_valid) state <= S_OUTPUT;  // waits for mac_sv; here it stays simplified")
    L.append(f"            end")
    L.append(f"            S_OUTPUT: begin")
    L.append(f"                // AH B-5 finish (K/V amortization): on a pending advance request,")
    L.append(f"                // recycle S_OUTPUT → S_RELOAD_Q (mirrors the S_WAIT_QK branch).")
    L.append(f"                if (pending_advance_in_b2b) begin")
    L.append(f"                    state <= S_RELOAD_Q;")
    L.append(f"                    mac_count <= 0;")
    L.append(f"                    row_count <= 0;")
    L.append(f"                    q_w_addr <= valid_q ? {addr_w_shared}'d1 : {addr_w_shared}'d0;")
    L.append(f"                    q_r_addr <= 0;")
    L.append(f"                end else if (!ready_n) state <= S_IDLE;")
    L.append(f"            end")
    L.append(f"            endcase")
    L.append(f"        end")
    L.append(f"    end")
    ...
    L.append(f"        );")
    L.append(f"        // AH Step 1: surface per-lane mac_qk valid_n to top-level array")
    ...
    L.append(f"        );")
    L.append(f"        // AH Step B-5: per-lane softmax_done = rising edge of clb_softmax's")
    L.append(f"        // valid_n (= entry into S_NORM, scores all captured + recip computed).")
    L.append(f"        reg attn_valid_d;")
    L.append(f"        always @(posedge clk or posedge rst) begin")
    L.append(f"            if (rst) attn_valid_d <= 0;")
    L.append(f"            else attn_valid_d <= attn_valid;")
    L.append(f"        end")
    L.append(f"        assign lane_softmax_done[lane] = attn_valid && !attn_valid_d;")
    L.append(f"")
    ...
    L.append(f"        // AH B-5 finish: gate mac_sv's valid input so it accumulates EXACTLY K={packed_N}")
    L.append(f"        // inputs per Q row, fires once, then disables until the next Q row's reload.")
    L.append(f"        // This matches expected_counters.json's fires_per_lane_per_row=1 contract.")
    L.append(f"        // Gate signal: counts weight_buf_valid pulses; goes low after K hits, re-arms")
    L.append(f"        // when the FSM enters S_RELOAD_Q (= per-Q-row reset).")
    L.append(f"        reg [{addr_w_shared}-1:0] mac_sv_input_count;")
    L.append(f"        always @(posedge clk or posedge rst) begin")
    L.append(f"            if (rst) mac_sv_input_count <= 0;")
    L.append(f"            else if (state == S_RELOAD_Q || state == S_LOAD)")
    L.append(f"                mac_sv_input_count <= 0;")
    L.append(f"            else if (weight_buf_valid && mac_sv_input_count < {packed_N})")
    L.append(f"                mac_sv_input_count <= mac_sv_input_count + 1;")
    L.append(f"        end")
    L.append(f"        wire mac_sv_valid_gated = weight_buf_valid && (mac_sv_input_count < {packed_N});")
    L.append(f"        dsp_mac #(.DATA_WIDTH(DATA_WIDTH), .K({packed_N})) mac_sv_inst (")
    L.append(f"            .clk(clk), .rst(rst),")
    L.append(f"            .valid(mac_sv_valid_gated), .ready_n(1'b0),")
    ...
    L.append(f"    // mac_qk dsp_mac. Cycles unchanged vs pre-Step-1 (purely additive).")
    L.append(f"    assign score_valid_o = |lane_score_valid;")
    L.append(f"    // AH Step B-5: AND-reduce score_valid → score_all_done_o.  AL has a")
    L.append(f"    // single shared FSM driving all W lanes' mac_qk in lockstep, so this is")
    L.append(f"    // mostly cosmetic, but exposes consistent interface across NL/AL.")
    L.append(f"    assign score_all_done_o = &lane_score_valid;")
    L.append(f"    // AH Step B-5: top-level done aggregation.")
    L.append(f"    //   softmax_done_o : AND-reduced rising-edge of softmax_inst.valid_n,")
    L.append(f"    //                    asserts when ALL W lanes' clb_softmax simultaneously")
    L.append(f"    //                    enter S_NORM (= scores all captured for this Q row).")
    L.append(f"    //   wsum_done_o    : AH B-5 finish: 1-cycle pulse on the FALLING edge of")
    L.append(f"    //                    ANY lane's clb_softmax valid_n (= S_NORM → S_LOAD)")
    L.append(f"    //                    AND lane[0]'s mac_sv produced a valid_n in this Q row.")
    L.append(f"    //                    Why this signal: clb_softmax must fully exit S_NORM")
    L.append(f"    //                    before the next Q row's mac_qk feeds new scores —")
    L.append(f"    //                    otherwise valid input pulses are dropped during the")
    L.append(f"    //                    existing S_NORM phase.  Firing the head's trigger on")
    L.append(f"    //                    S_NORM exit guarantees back-pressure across Q rows.")
    L.append(f"    assign softmax_done_o = &lane_softmax_done;")
    L.append(f"    // Detect ANY lane's clb_softmax S_NORM → S_LOAD transition.  The lanes are")
    L.append(f"    // lock-stepped (fed by the same mac_qk valid pulses), so their valid_n falls")
    L.append(f"    // simultaneously.  Edge-detect on lane[0]'s valid_n (sufficient for parity).")
    L.append(f"    reg lane0_attn_valid_d;")
    L.append(f"    wire lane0_attn_valid = al_lane[0].softmax_inst.valid_n;")
    L.append(f"    always @(posedge clk or posedge rst) begin")
    L.append(f"        if (rst) lane0_attn_valid_d <= 1'b0;")
    L.append(f"        else lane0_attn_valid_d <= lane0_attn_valid;")
    L.append(f"    end")
    L.append(f"    // Pulse on falling edge: was 1 last cycle, now 0 = S_NORM finished.")
    L.append(f"    assign wsum_done_o = lane0_attn_valid_d && !lane0_attn_valid;")
    L.append(f"")
