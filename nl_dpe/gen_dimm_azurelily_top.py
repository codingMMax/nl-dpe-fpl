...
def _gen_dimm_top_azurelily(n_seq, d_head, crossbar_cols, W, data_width=40):
    ...
    L.append(f"                // AH Step B-2 (additive, default off): when SCORE_BACK_TO_BACK_MODE=1")
    L.append(f"                // and the head FSM (B-5) pulses score_next_q_row_trigger_i, recycle")
    L.append(f"                // S_WAIT_QK → S_FEED_QK to begin computing scores for the next Q row.")
    L.append(f"                // Default mode=0 keeps the canonical S_WAIT_QK → S_OUTPUT path.")
    L.append(f"                if (SCORE_BACK_TO_BACK_MODE && score_next_q_row_trigger_i) begin")
    L.append(f"                    state <= S_FEED_QK;")
    L.append(f"                    mac_count <= 0;")
    L.append(f"                    row_count <= 0;")
    L.append(f"                end else if (|lane_valid) state <= S_OUTPUT;  // waits for mac_sv; here it stays simplified")
    L.append(f"            end")
    L.append(f"            S_OUTPUT: begin")
    L.append(f"                // AH Step B-3 (additive, default off): when WSUM_BACK_TO_BACK_MODE=1")
    L.append(f"                // and head FSM pulses wsum_next_q_row_trigger_i, return to S_FEED_QK")
    L.append(f"                // to re-arm the dsp_mac auto-recycle for the next Q row's mac_sv. The")
    L.append(f"                // dsp_mac instances themselves auto-recycle (count → 0 after K=32);")
    L.append(f"                // this transition only re-engages the upstream FSM driving the data.")
    L.append(f"                // Default mode=0 keeps the canonical S_OUTPUT → S_IDLE behaviour.")
    L.append(f"                if (WSUM_BACK_TO_BACK_MODE && wsum_next_q_row_trigger_i) begin")
    L.append(f"                    state <= S_FEED_QK;")
    L.append(f"                    mac_count <= 0;")
    L.append(f"                    row_count <= 0;")
    ...
    L.append(f"    //   wsum_done_o    : OR-reduced lane_valid (mac_sv valid_n).  Asserts the")
    L.append(f"    //                    cycle the FIRST lane's mac_sv produces a valid output,")
    L.append(f"    //                    matching sim's gemm_dsp(fires_per_lane_per_row=1) FSM-")
    L.append(f"    //                    cutoff convention.  Used by the head FSM to advance Q rows.")
    L.append(f"    assign softmax_done_o = &lane_softmax_done;")
    L.append(f"    assign wsum_done_o    = |lane_valid;")
    L.append(f"")
