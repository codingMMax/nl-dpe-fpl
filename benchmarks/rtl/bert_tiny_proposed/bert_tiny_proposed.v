...
// FFN1: V=1 H=4 → 4 DPEs
// FFN2: V=1 H=1 → 1 DPEs
// DIMM: H_dimm=1 → 4 DPEs/head × 2 heads × 2 blocks = 16
// ALL layers V=1 → ACAM handles activation (NL-DPE) / CLB activation (Azure-Lily)

module bert_tiny_proposed (
    ...
    // Head 0: DIMM score matrix (DPE I|exp + CLB add/reduce)
    dimm_score_matrix_b0 #(.DATA_WIDTH(40)) b0_h0_score_inst (
        .clk(clk), .rst(rst),
        .valid_q(valid_b0_q), .valid_k(valid_b0_k),
        .ready_n(ready_b0_h0_score),
    ...
    // Head 0: Softmax (DPE I|exp + CLB sum/recip/mul)
    softmax_approx_b0 #(.DATA_WIDTH(40)) b0_h0_softmax_inst (
        .clk(clk), .rst(rst),
        .valid(valid_b0_h0_score), .ready_n(ready_b0_h0_softmax),
        .data_in(data_b0_h0_score),
        .data_out(data_b0_h0_softmax), .ready(ready_b0_h0_score), .valid_n(valid_b0_h0_softmax)
    );
    // Head 0: DIMM weighted sum (DPE I|log + CLB add + DPE I|exp)
    dimm_weighted_sum_b0 #(.DATA_WIDTH(40)) b0_h0_wsum_inst (
        .clk(clk), .rst(rst),
        .valid_attn(valid_b0_h0_softmax), .valid_v(valid_b0_v),
        .ready_n(ready_b0_h0_wsum),
    ...
    // Head 1: DIMM score matrix (DPE I|exp + CLB add/reduce)
    dimm_score_matrix_b0 #(.DATA_WIDTH(40)) b0_h1_score_inst (
        .clk(clk), .rst(rst),
        .valid_q(valid_b0_q), .valid_k(valid_b0_k),
        .ready_n(ready_b0_h1_score),
        .data_in_q(data_b0_q), .data_in_k(data_b0_k),
        .data_out(data_b0_h1_score),
        .ready_q(ready_b0_h1_score_q), .ready_k(ready_b0_h1_score_k),
        .valid_n(valid_b0_h1_score)
    );
    // Head 1: Softmax (DPE I|exp + CLB sum/recip/mul)
    softmax_approx_b0 #(.DATA_WIDTH(40)) b0_h1_softmax_inst (
        .clk(clk), .rst(rst),
        .valid(valid_b0_h1_score), .ready_n(ready_b0_h1_softmax),
        .data_in(data_b0_h1_score),
        .data_out(data_b0_h1_softmax), .ready(ready_b0_h1_score), .valid_n(valid_b0_h1_softmax)
    );
    // Head 1: DIMM weighted sum (DPE I|log + CLB add + DPE I|exp)
    dimm_weighted_sum_b0 #(.DATA_WIDTH(40)) b0_h1_wsum_inst (
        .clk(clk), .rst(rst),
        .valid_attn(valid_b0_h1_softmax), .valid_v(valid_b0_v),
        .ready_n(ready_b0_h1_wsum),
    ...
    // Head 0: DIMM score matrix (DPE I|exp + CLB add/reduce)
    dimm_score_matrix_b1 #(.DATA_WIDTH(40)) b1_h0_score_inst (
        .clk(clk), .rst(rst),
        .valid_q(valid_b1_q), .valid_k(valid_b1_k),
        .ready_n(ready_b1_h0_score),
    ...
    // Head 0: Softmax (DPE I|exp + CLB sum/recip/mul)
    softmax_approx_b1 #(.DATA_WIDTH(40)) b1_h0_softmax_inst (
        .clk(clk), .rst(rst),
        .valid(valid_b1_h0_score), .ready_n(ready_b1_h0_softmax),
        .data_in(data_b1_h0_score),
        .data_out(data_b1_h0_softmax), .ready(ready_b1_h0_score), .valid_n(valid_b1_h0_softmax)
    );
    // Head 0: DIMM weighted sum (DPE I|log + CLB add + DPE I|exp)
    dimm_weighted_sum_b1 #(.DATA_WIDTH(40)) b1_h0_wsum_inst (
        .clk(clk), .rst(rst),
        .valid_attn(valid_b1_h0_softmax), .valid_v(valid_b1_v),
        .ready_n(ready_b1_h0_wsum),
    ...
    // Head 1: DIMM score matrix (DPE I|exp + CLB add/reduce)
    dimm_score_matrix_b1 #(.DATA_WIDTH(40)) b1_h1_score_inst (
        .clk(clk), .rst(rst),
        .valid_q(valid_b1_q), .valid_k(valid_b1_k),
        .ready_n(ready_b1_h1_score),
        .data_in_q(data_b1_q), .data_in_k(data_b1_k),
    ...
    // Head 1: Softmax (DPE I|exp + CLB sum/recip/mul)
    softmax_approx_b1 #(.DATA_WIDTH(40)) b1_h1_softmax_inst (
        .clk(clk), .rst(rst),
        .valid(valid_b1_h1_score), .ready_n(ready_b1_h1_softmax),
        .data_in(data_b1_h1_score),
        .data_out(data_b1_h1_softmax), .ready(ready_b1_h1_score), .valid_n(valid_b1_h1_softmax)
    );
    // Head 1: DIMM weighted sum (DPE I|log + CLB add + DPE I|exp)
    dimm_weighted_sum_b1 #(.DATA_WIDTH(40)) b1_h1_wsum_inst (
        .clk(clk), .rst(rst),
        .valid_attn(valid_b1_h1_softmax), .valid_v(valid_b1_v),
        .ready_n(ready_b1_h1_wsum),
...
// ═══════════════════════════════════════
// DIMM Modules — Block 0
// ═══════════════════════════════════════
module dimm_score_matrix_b0 #(
    parameter N = 128,
    parameter d = 64,
    parameter DATA_WIDTH = 40,
    ...
    k_sram (.clk(clk),.rst(rst),.w_en(k_w_en),.r_addr(k_read_addr[$clog2(N*d)-1:0]),
            .w_addr(k_write_addr[$clog2(N*d)-1:0]),.sram_data_in(data_in_k),.sram_data_out(k_sram_out));

    // CLB adder: log-domain addition (replaces multiplication)
    wire [DATA_WIDTH-1:0] log_sum = q_sram_out + k_sram_out;

    // DPE(I|exp) stage: identity crossbar + ACAM=exp
    ...
    assign dimm_exp_data_in = log_sum;
    ...
    always @(posedge clk) begin
        ...
        else if (dimm_exp_valid_n) accumulator <= accumulator + dimm_exp_data_out;
    end
    ...
    always @(posedge clk or posedge rst) begin
        if (rst) begin
            ...
            k_write_addr <= 0; k_read_addr <= 0; k_w_en <= 0;
            ...
            case (state)
                ...
                S_COMPUTE: begin
                    // Feed log_sum to DPE, accumulate exp results
    ...
    always @(posedge clk or posedge rst) begin
        if (rst) begin
            ...
            case (sm_state)
                ...
                SM_NORMALIZE: begin
                    // CLB multiply: exp_val × inv_sum (normalization, not Taylor)
...
// ═══════════════════════════════════════
// DIMM Modules — Block 1
// ═══════════════════════════════════════
module dimm_score_matrix_b1 #(
    parameter N = 128,
    parameter d = 64,
    parameter DATA_WIDTH = 40,
    parameter ADDR_WIDTH = 13,
