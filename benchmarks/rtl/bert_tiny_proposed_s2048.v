...
// DIMM: 64 DPEs/head × 2 heads × 2 blocks = 256

module bert_tiny_proposed_s2048 (
    ...
    // 256 DPEs for DIMM stages across all blocks/heads
...
//   Q=526, K=26728, score=923, row=923, V=26728, out=526
module dimm_score_matrix_b0_h0 #(
    parameter N = 2048,
    parameter d = 64,
    parameter DATA_WIDTH = 40,
    parameter ADDR_WIDTH = 15,
    parameter DEPTH = 26728
)(
    ...
    reg [15-1:0] k_write_addr, k_read_addr_a;
    reg k_w_en;
    wire [DATA_WIDTH-1:0] k_sram_out_a;
    sram #(.N_CHANNELS(1),.DATA_WIDTH(DATA_WIDTH),.DEPTH(26728))
    k_sram_a (.clk(clk),.rst(rst),.w_en(k_w_en),.r_addr(k_read_addr_a[15-1:0]),
              .w_addr(k_write_addr[15-1:0]),.sram_data_in(data_in_k),.sram_data_out(k_sram_out_a));
    reg [15-1:0] k_read_addr_b;
    wire [DATA_WIDTH-1:0] k_sram_out_b;
    sram #(.N_CHANNELS(1),.DATA_WIDTH(DATA_WIDTH),.DEPTH(26728))
    k_sram_b (.clk(clk),.rst(rst),.w_en(k_w_en),.r_addr(k_read_addr_b[15-1:0]),
              .w_addr(k_write_addr[15-1:0]),.sram_data_in(data_in_k),.sram_data_out(k_sram_out_b));
    ...
    sram #(.N_CHANNELS(1),.DATA_WIDTH(DATA_WIDTH),.DEPTH(923))
...
module softmax_approx_b0_h0 #(
    ...
    sram #(.N_CHANNELS(1),.DATA_WIDTH(DATA_WIDTH),.DEPTH(924))
    ...
    sram #(.N_CHANNELS(1),.DATA_WIDTH(DATA_WIDTH),.DEPTH(925))
...
module dimm_weighted_sum_b0_h0 #(
    ...
    sram #(.N_CHANNELS(1),.DATA_WIDTH(DATA_WIDTH),.DEPTH(926))
    ...
    reg [15-1:0] v_write_addr, v_read_addr;
    reg v_w_en;
    wire [DATA_WIDTH-1:0] v_sram_out;
    sram #(.N_CHANNELS(1),.DATA_WIDTH(DATA_WIDTH),.DEPTH(26728))
    v_sram (.clk(clk),.rst(rst),.w_en(v_w_en),.r_addr(v_read_addr[15-1:0]),
            .w_addr(v_write_addr[15-1:0]),.sram_data_in(data_in_v),.sram_data_out(v_sram_out));
    ...
    sram #(.N_CHANNELS(1),.DATA_WIDTH(DATA_WIDTH),.DEPTH(927))
...
//   Q=1039, K=27241, score=1436, row=1436, V=27241, out=1039
module dimm_score_matrix_b0_h1 #(
    parameter N = 2048,
    parameter d = 64,
    parameter DATA_WIDTH = 40,
    parameter ADDR_WIDTH = 15,
    parameter DEPTH = 27241
)(
    input wire clk, input wire rst,
    input wire valid_q, input wire valid_k,
    input wire ready_n,
    ...
    reg k_w_en;
    wire [DATA_WIDTH-1:0] k_sram_out_a;
    sram #(.N_CHANNELS(1),.DATA_WIDTH(DATA_WIDTH),.DEPTH(27241))
    k_sram_a (.clk(clk),.rst(rst),.w_en(k_w_en),.r_addr(k_read_addr_a[15-1:0]),
              .w_addr(k_write_addr[15-1:0]),.sram_data_in(data_in_k),.sram_data_out(k_sram_out_a));
    reg [15-1:0] k_read_addr_b;
    wire [DATA_WIDTH-1:0] k_sram_out_b;
    ...
    reg [DATA_WIDTH-1:0] score_write_data;
    wire [DATA_WIDTH-1:0] score_sram_out;
    sram #(.N_CHANNELS(1),.DATA_WIDTH(DATA_WIDTH),.DEPTH(1436))
    score_sram (.clk(clk),.rst(rst),.w_en(score_w_en),.r_addr(score_read_addr),
                .w_addr(score_write_addr),.sram_data_in(score_write_data),.sram_data_out(score_sram_out));
...
module softmax_approx_b0_h1 #(
    ...
    reg [DATA_WIDTH-1:0] exp_write_data;
    wire [DATA_WIDTH-1:0] exp_sram_out;
    sram #(.N_CHANNELS(1),.DATA_WIDTH(DATA_WIDTH),.DEPTH(1437))
    exp_sram (.clk(clk),.rst(rst),.w_en(exp_w_en),.r_addr(exp_read_addr),
              .w_addr(exp_write_addr),.sram_data_in(exp_write_data),.sram_data_out(exp_sram_out));
    reg [2*DATA_WIDTH-1:0] exp_sum;
    ...
    reg out_w_en;
    wire [DATA_WIDTH-1:0] out_sram_out;
    sram #(.N_CHANNELS(1),.DATA_WIDTH(DATA_WIDTH),.DEPTH(1438))
    out_sram (.clk(clk),.rst(rst),.w_en(out_w_en),.r_addr(out_read_addr),
              .w_addr(out_write_addr),.sram_data_in(norm_val),.sram_data_out(out_sram_out));
...
module dimm_weighted_sum_b0_h1 #(
    ...
    reg attn_w_en;
    wire [DATA_WIDTH-1:0] attn_sram_out;
    sram #(.N_CHANNELS(1),.DATA_WIDTH(DATA_WIDTH),.DEPTH(1439))
    attn_sram (.clk(clk),.rst(rst),.w_en(attn_w_en),.r_addr(attn_read_addr),
               .w_addr(attn_write_addr),.sram_data_in(data_in_attn),.sram_data_out(attn_sram_out));
    ...
    reg [DATA_WIDTH-1:0] log_attn_write_data;
    wire [DATA_WIDTH-1:0] log_attn_sram_out;
    sram #(.N_CHANNELS(1),.DATA_WIDTH(DATA_WIDTH),.DEPTH(1440))
    log_attn_sram (.clk(clk),.rst(rst),.w_en(log_attn_w_en),.r_addr(log_attn_read_addr),
                   .w_addr(log_attn_write_addr),.sram_data_in(log_attn_write_data),.sram_data_out(log_attn_sram_out));
...
//   Q=1552, K=27754, score=1949, row=1949, V=27754, out=1552
module dimm_score_matrix_b1_h0 #(
    parameter N = 2048,
    parameter d = 64,
    parameter DATA_WIDTH = 40,
    parameter ADDR_WIDTH = 15,
    parameter DEPTH = 27754
)(
    input wire clk, input wire rst,
    input wire valid_q, input wire valid_k,
    input wire ready_n,
    ...
    reg k_w_en;
    wire [DATA_WIDTH-1:0] k_sram_out_a;
    sram #(.N_CHANNELS(1),.DATA_WIDTH(DATA_WIDTH),.DEPTH(27754))
    k_sram_a (.clk(clk),.rst(rst),.w_en(k_w_en),.r_addr(k_read_addr_a[15-1:0]),
              .w_addr(k_write_addr[15-1:0]),.sram_data_in(data_in_k),.sram_data_out(k_sram_out_a));
    reg [15-1:0] k_read_addr_b;
    wire [DATA_WIDTH-1:0] k_sram_out_b;
    ...
    reg [DATA_WIDTH-1:0] score_write_data;
    wire [DATA_WIDTH-1:0] score_sram_out;
    sram #(.N_CHANNELS(1),.DATA_WIDTH(DATA_WIDTH),.DEPTH(1949))
    score_sram (.clk(clk),.rst(rst),.w_en(score_w_en),.r_addr(score_read_addr),
                .w_addr(score_write_addr),.sram_data_in(score_write_data),.sram_data_out(score_sram_out));
...
module softmax_approx_b1_h0 #(
    ...
    reg [DATA_WIDTH-1:0] exp_write_data;
    wire [DATA_WIDTH-1:0] exp_sram_out;
    sram #(.N_CHANNELS(1),.DATA_WIDTH(DATA_WIDTH),.DEPTH(1950))
    exp_sram (.clk(clk),.rst(rst),.w_en(exp_w_en),.r_addr(exp_read_addr),
              .w_addr(exp_write_addr),.sram_data_in(exp_write_data),.sram_data_out(exp_sram_out));
    reg [2*DATA_WIDTH-1:0] exp_sum;
    ...
    reg out_w_en;
    wire [DATA_WIDTH-1:0] out_sram_out;
    sram #(.N_CHANNELS(1),.DATA_WIDTH(DATA_WIDTH),.DEPTH(1951))
    out_sram (.clk(clk),.rst(rst),.w_en(out_w_en),.r_addr(out_read_addr),
              .w_addr(out_write_addr),.sram_data_in(norm_val),.sram_data_out(out_sram_out));
...
module dimm_weighted_sum_b1_h0 #(
    ...
    reg attn_w_en;
    wire [DATA_WIDTH-1:0] attn_sram_out;
    sram #(.N_CHANNELS(1),.DATA_WIDTH(DATA_WIDTH),.DEPTH(1952))
    attn_sram (.clk(clk),.rst(rst),.w_en(attn_w_en),.r_addr(attn_read_addr),
               .w_addr(attn_write_addr),.sram_data_in(data_in_attn),.sram_data_out(attn_sram_out));
    ...
    reg [DATA_WIDTH-1:0] log_attn_write_data;
    wire [DATA_WIDTH-1:0] log_attn_sram_out;
    sram #(.N_CHANNELS(1),.DATA_WIDTH(DATA_WIDTH),.DEPTH(1953))
    log_attn_sram (.clk(clk),.rst(rst),.w_en(log_attn_w_en),.r_addr(log_attn_read_addr),
                   .w_addr(log_attn_write_addr),.sram_data_in(log_attn_write_data),.sram_data_out(log_attn_sram_out));
...
//   Q=2065, K=28267, score=2462, row=2462, V=28267, out=2065
module dimm_score_matrix_b1_h1 #(
    parameter N = 2048,
    parameter d = 64,
    parameter DATA_WIDTH = 40,
    parameter ADDR_WIDTH = 15,
    parameter DEPTH = 28267
)(
    input wire clk, input wire rst,
    input wire valid_q, input wire valid_k,
    input wire ready_n,
    ...
    reg k_w_en;
    wire [DATA_WIDTH-1:0] k_sram_out_a;
    sram #(.N_CHANNELS(1),.DATA_WIDTH(DATA_WIDTH),.DEPTH(28267))
    k_sram_a (.clk(clk),.rst(rst),.w_en(k_w_en),.r_addr(k_read_addr_a[15-1:0]),
              .w_addr(k_write_addr[15-1:0]),.sram_data_in(data_in_k),.sram_data_out(k_sram_out_a));
    reg [15-1:0] k_read_addr_b;
    wire [DATA_WIDTH-1:0] k_sram_out_b;
    ...
    reg [DATA_WIDTH-1:0] score_write_data;
    wire [DATA_WIDTH-1:0] score_sram_out;
    sram #(.N_CHANNELS(1),.DATA_WIDTH(DATA_WIDTH),.DEPTH(2462))
    score_sram (.clk(clk),.rst(rst),.w_en(score_w_en),.r_addr(score_read_addr),
                .w_addr(score_write_addr),.sram_data_in(score_write_data),.sram_data_out(score_sram_out));
...
module softmax_approx_b1_h1 #(
    ...
    reg [DATA_WIDTH-1:0] exp_write_data;
    wire [DATA_WIDTH-1:0] exp_sram_out;
    sram #(.N_CHANNELS(1),.DATA_WIDTH(DATA_WIDTH),.DEPTH(2463))
    exp_sram (.clk(clk),.rst(rst),.w_en(exp_w_en),.r_addr(exp_read_addr),
              .w_addr(exp_write_addr),.sram_data_in(exp_write_data),.sram_data_out(exp_sram_out));
    reg [2*DATA_WIDTH-1:0] exp_sum;
    ...
    reg out_w_en;
    wire [DATA_WIDTH-1:0] out_sram_out;
    sram #(.N_CHANNELS(1),.DATA_WIDTH(DATA_WIDTH),.DEPTH(2464))
    out_sram (.clk(clk),.rst(rst),.w_en(out_w_en),.r_addr(out_read_addr),
              .w_addr(out_write_addr),.sram_data_in(norm_val),.sram_data_out(out_sram_out));
...
module dimm_weighted_sum_b1_h1 #(
    ...
    reg attn_w_en;
    wire [DATA_WIDTH-1:0] attn_sram_out;
    sram #(.N_CHANNELS(1),.DATA_WIDTH(DATA_WIDTH),.DEPTH(2465))
    attn_sram (.clk(clk),.rst(rst),.w_en(attn_w_en),.r_addr(attn_read_addr),
               .w_addr(attn_write_addr),.sram_data_in(data_in_attn),.sram_data_out(attn_sram_out));
    ...
    reg [DATA_WIDTH-1:0] log_attn_write_data;
    wire [DATA_WIDTH-1:0] log_attn_sram_out;
    sram #(.N_CHANNELS(1),.DATA_WIDTH(DATA_WIDTH),.DEPTH(2466))
    log_attn_sram (.clk(clk),.rst(rst),.w_en(log_attn_w_en),.r_addr(log_attn_read_addr),
                   .w_addr(log_attn_write_addr),.sram_data_in(log_attn_write_data),.sram_data_out(log_attn_sram_out));
