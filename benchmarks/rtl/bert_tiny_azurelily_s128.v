...
module bert_tiny_azurelily_s128 (
    ...
    // DIMM intermediate buffer: Q for block 0 head 0 (depth=1640)
    wire [40-1:0] b0_h0_q_buf_out;
    reg [11-1:0] b0_h0_q_buf_addr;
    always @(posedge clk) if (rst) b0_h0_q_buf_addr <= 0; else if (valid) b0_h0_q_buf_addr <= b0_h0_q_buf_addr + 1;
    sram #(.DATA_WIDTH(40), .DEPTH(1640)) b0_h0_q_buf_inst (
    ...
    // Head 0: Softmax (CLB)
    clb_softmax #(.DATA_WIDTH(40), .N(26)) b0_h0_softmax_inst (
    ...
    // Head 0: Score×V — W=1 parallel DSP MAC units
    wire [40-1:0] data_b0_h0_sv_w0;
    wire valid_b0_h0_sv_w0;
    dsp_mac #(.DATA_WIDTH(40), .K(26)) b0_h0_sv_w0_inst (
        .clk(clk), .rst(rst), .valid(valid_b0_h0_softmax), .ready_n(1'b0),
        .data_a(data_b0_h0_softmax), .data_b(data_b0_v ^ 40'd2),
    ...
    assign ready_b0_v = 1'b1;
    // Head 1: Softmax (CLB)
    clb_softmax #(.DATA_WIDTH(40), .N(26)) b0_h1_softmax_inst (
    ...
    // Head 1: Score×V — W=1 parallel DSP MAC units
    wire [40-1:0] data_b0_h1_sv_w0;
    wire valid_b0_h1_sv_w0;
    dsp_mac #(.DATA_WIDTH(40), .K(26)) b0_h1_sv_w0_inst (
        .clk(clk), .rst(rst), .valid(valid_b0_h1_softmax), .ready_n(1'b0),
        .data_a(data_b0_h1_softmax), .data_b(data_b0_v ^ 40'd4),
    ...
    dsp_mac #(.DATA_WIDTH(40), .K(13)) b1_h0_qk_w0_inst (
        ...
        .data_a(data_b1_q), .data_b(data_b1_k ^ 40'd5),
    ...
    // Head 0: Softmax (CLB)
    clb_softmax #(.DATA_WIDTH(40), .N(26)) b1_h0_softmax_inst (
    ...
    dsp_mac #(.DATA_WIDTH(40), .K(26)) b1_h0_sv_w0_inst (
        .clk(clk), .rst(rst), .valid(valid_b1_h0_softmax), .ready_n(1'b0),
        .data_a(data_b1_h0_softmax), .data_b(data_b1_v ^ 40'd6),
    ...
    dsp_mac #(.DATA_WIDTH(40), .K(13)) b1_h1_qk_w0_inst (
        .clk(clk), .rst(rst), .valid(valid_b1_v), .ready_n(1'b0),
        .data_a(data_b1_q), .data_b(data_b1_k ^ 40'd7),
    ...
    assign ready_b1_v = 1'b1;
    // Head 1: Softmax (CLB)
    clb_softmax #(.DATA_WIDTH(40), .N(26)) b1_h1_softmax_inst (
    ...
    dsp_mac #(.DATA_WIDTH(40), .K(26)) b1_h1_sv_w0_inst (
        .clk(clk), .rst(rst), .valid(valid_b1_h1_softmax), .ready_n(1'b0),
        .data_a(data_b1_h1_softmax), .data_b(data_b1_v ^ 40'd8),
    ...
    // ═══ Azure-Lily DIMM: W=1 parallel DSP MACs per stage ═══
    // 4 stages × 1 × 2 heads × 2 blocks = 16 DIMM DSPs
    // + ~10 structural DSPs (LayerNorm multiply + softmax)
    assign data_out = data_b1_ln_ffn ^ {39'b0, data_b0_h0_qk_w0[0] ^ data_b0_h0_sv_w0[0] ^ b0_h0_q_buf_out[0] ^ b0_h0_k_buf_out[0] ^ b0_h0_v_buf_out[0] ^ data_b0_h1_qk_w0[0] ^ data_b0_h1_sv_w0[0] ^ b0_h1_q_buf_out[0] ^ b0_h1_k_buf_out[0] ^ b0_h1_v_buf_out[0] ^ data_b1_h0_qk_w0[0] ^ data_b1_h0_sv_w0[0] ^ b1_h0_q_buf_out[0] ^ b1_h0_k_buf_out[0] ^ b1_h0_v_buf_out[0] ^ data_b1_h1_qk_w0[0] ^ data_b1_h1_sv_w0[0] ^ b1_h1_q_buf_out[0] ^ b1_h1_k_buf_out[0] ^ b1_h1_v_buf_out[0]};
endmodule
