...
module bert_tiny_proposed_s1024 (
    ...
    sram #(.DATA_WIDTH(40), .DEPTH(237)) b1_h1_dimm_la_inst (
        .clk(clk), .rst(rst), .w_en(valid),
...
//   Q=526, K=13621, score=718, row=718, V=13621, out=526
module dimm_score_matrix_b0_h0 #(
    parameter N = 1024,
    parameter d = 64,
    parameter DATA_WIDTH = 40,
    parameter ADDR_WIDTH = 14,
    parameter DEPTH = 13621
)(
    ...
    sram #(.N_CHANNELS(1),.DATA_WIDTH(DATA_WIDTH),.DEPTH(13621))
    ...
    sram #(.N_CHANNELS(1),.DATA_WIDTH(DATA_WIDTH),.DEPTH(718))
...
module softmax_approx_b0_h0 #(
    ...
    sram #(.N_CHANNELS(1),.DATA_WIDTH(DATA_WIDTH),.DEPTH(719))
    ...
    sram #(.N_CHANNELS(1),.DATA_WIDTH(DATA_WIDTH),.DEPTH(720))
...
module dimm_weighted_sum_b0_h0 #(
    ...
    sram #(.N_CHANNELS(1),.DATA_WIDTH(DATA_WIDTH),.DEPTH(721))
    ...
    sram #(.N_CHANNELS(1),.DATA_WIDTH(DATA_WIDTH),.DEPTH(722))
...
//   Q=1039, K=14134, score=1231, row=1231, V=14134, out=1039
module dimm_score_matrix_b0_h1 #(
    parameter N = 1024,
    parameter d = 64,
    parameter DATA_WIDTH = 40,
    parameter ADDR_WIDTH = 14,
    parameter DEPTH = 14134
)(
    input wire clk, input wire rst,
    input wire valid_q, input wire valid_k,
    input wire ready_n,
    ...
    reg k_w_en;
    wire [DATA_WIDTH-1:0] k_sram_out_a;
    sram #(.N_CHANNELS(1),.DATA_WIDTH(DATA_WIDTH),.DEPTH(14134))
    k_sram_a (.clk(clk),.rst(rst),.w_en(k_w_en),.r_addr(k_read_addr_a[14-1:0]),
              .w_addr(k_write_addr[14-1:0]),.sram_data_in(data_in_k),.sram_data_out(k_sram_out_a));
    reg [14-1:0] k_read_addr_b;
    wire [DATA_WIDTH-1:0] k_sram_out_b;
    ...
    reg [DATA_WIDTH-1:0] score_write_data;
    wire [DATA_WIDTH-1:0] score_sram_out;
    sram #(.N_CHANNELS(1),.DATA_WIDTH(DATA_WIDTH),.DEPTH(1231))
    score_sram (.clk(clk),.rst(rst),.w_en(score_w_en),.r_addr(score_read_addr),
                .w_addr(score_write_addr),.sram_data_in(score_write_data),.sram_data_out(score_sram_out));
...
module softmax_approx_b0_h1 #(
    ...
    reg [DATA_WIDTH-1:0] exp_write_data;
    wire [DATA_WIDTH-1:0] exp_sram_out;
    sram #(.N_CHANNELS(1),.DATA_WIDTH(DATA_WIDTH),.DEPTH(1232))
    exp_sram (.clk(clk),.rst(rst),.w_en(exp_w_en),.r_addr(exp_read_addr),
              .w_addr(exp_write_addr),.sram_data_in(exp_write_data),.sram_data_out(exp_sram_out));
    reg [2*DATA_WIDTH-1:0] exp_sum;
    ...
    reg out_w_en;
    wire [DATA_WIDTH-1:0] out_sram_out;
    sram #(.N_CHANNELS(1),.DATA_WIDTH(DATA_WIDTH),.DEPTH(1233))
    out_sram (.clk(clk),.rst(rst),.w_en(out_w_en),.r_addr(out_read_addr),
              .w_addr(out_write_addr),.sram_data_in(norm_val),.sram_data_out(out_sram_out));
...
module dimm_weighted_sum_b0_h1 #(
    ...
    reg attn_w_en;
    wire [DATA_WIDTH-1:0] attn_sram_out;
    sram #(.N_CHANNELS(1),.DATA_WIDTH(DATA_WIDTH),.DEPTH(1234))
    attn_sram (.clk(clk),.rst(rst),.w_en(attn_w_en),.r_addr(attn_read_addr),
               .w_addr(attn_write_addr),.sram_data_in(data_in_attn),.sram_data_out(attn_sram_out));
    ...
    reg [DATA_WIDTH-1:0] log_attn_write_data;
    wire [DATA_WIDTH-1:0] log_attn_sram_out;
    sram #(.N_CHANNELS(1),.DATA_WIDTH(DATA_WIDTH),.DEPTH(1235))
    log_attn_sram (.clk(clk),.rst(rst),.w_en(log_attn_w_en),.r_addr(log_attn_read_addr),
                   .w_addr(log_attn_write_addr),.sram_data_in(log_attn_write_data),.sram_data_out(log_attn_sram_out));
...
//   Q=1552, K=14647, score=1744, row=1744, V=14647, out=1552
module dimm_score_matrix_b1_h0 #(
    parameter N = 1024,
    parameter d = 64,
    parameter DATA_WIDTH = 40,
    parameter ADDR_WIDTH = 14,
    parameter DEPTH = 14647
)(
    input wire clk, input wire rst,
    input wire valid_q, input wire valid_k,
    input wire ready_n,
    ...
    reg k_w_en;
    wire [DATA_WIDTH-1:0] k_sram_out_a;
    sram #(.N_CHANNELS(1),.DATA_WIDTH(DATA_WIDTH),.DEPTH(14647))
    k_sram_a (.clk(clk),.rst(rst),.w_en(k_w_en),.r_addr(k_read_addr_a[14-1:0]),
              .w_addr(k_write_addr[14-1:0]),.sram_data_in(data_in_k),.sram_data_out(k_sram_out_a));
    reg [14-1:0] k_read_addr_b;
    wire [DATA_WIDTH-1:0] k_sram_out_b;
    ...
    reg [DATA_WIDTH-1:0] score_write_data;
    wire [DATA_WIDTH-1:0] score_sram_out;
    sram #(.N_CHANNELS(1),.DATA_WIDTH(DATA_WIDTH),.DEPTH(1744))
    score_sram (.clk(clk),.rst(rst),.w_en(score_w_en),.r_addr(score_read_addr),
                .w_addr(score_write_addr),.sram_data_in(score_write_data),.sram_data_out(score_sram_out));
...
module softmax_approx_b1_h0 #(
    ...
    reg [DATA_WIDTH-1:0] exp_write_data;
    wire [DATA_WIDTH-1:0] exp_sram_out;
    sram #(.N_CHANNELS(1),.DATA_WIDTH(DATA_WIDTH),.DEPTH(1745))
    exp_sram (.clk(clk),.rst(rst),.w_en(exp_w_en),.r_addr(exp_read_addr),
              .w_addr(exp_write_addr),.sram_data_in(exp_write_data),.sram_data_out(exp_sram_out));
    reg [2*DATA_WIDTH-1:0] exp_sum;
    ...
    reg out_w_en;
    wire [DATA_WIDTH-1:0] out_sram_out;
    sram #(.N_CHANNELS(1),.DATA_WIDTH(DATA_WIDTH),.DEPTH(1746))
    out_sram (.clk(clk),.rst(rst),.w_en(out_w_en),.r_addr(out_read_addr),
              .w_addr(out_write_addr),.sram_data_in(norm_val),.sram_data_out(out_sram_out));
...
module dimm_weighted_sum_b1_h0 #(
    ...
    reg attn_w_en;
    wire [DATA_WIDTH-1:0] attn_sram_out;
    sram #(.N_CHANNELS(1),.DATA_WIDTH(DATA_WIDTH),.DEPTH(1747))
    attn_sram (.clk(clk),.rst(rst),.w_en(attn_w_en),.r_addr(attn_read_addr),
               .w_addr(attn_write_addr),.sram_data_in(data_in_attn),.sram_data_out(attn_sram_out));
    ...
    reg [DATA_WIDTH-1:0] log_attn_write_data;
    wire [DATA_WIDTH-1:0] log_attn_sram_out;
    sram #(.N_CHANNELS(1),.DATA_WIDTH(DATA_WIDTH),.DEPTH(1748))
    log_attn_sram (.clk(clk),.rst(rst),.w_en(log_attn_w_en),.r_addr(log_attn_read_addr),
                   .w_addr(log_attn_write_addr),.sram_data_in(log_attn_write_data),.sram_data_out(log_attn_sram_out));
...
//   Q=2065, K=15160, score=2257, row=2257, V=15160, out=2065
module dimm_score_matrix_b1_h1 #(
    parameter N = 1024,
    parameter d = 64,
    parameter DATA_WIDTH = 40,
    parameter ADDR_WIDTH = 14,
    parameter DEPTH = 15160
)(
    input wire clk, input wire rst,
    input wire valid_q, input wire valid_k,
    input wire ready_n,
    ...
    reg k_w_en;
    wire [DATA_WIDTH-1:0] k_sram_out_a;
    sram #(.N_CHANNELS(1),.DATA_WIDTH(DATA_WIDTH),.DEPTH(15160))
    k_sram_a (.clk(clk),.rst(rst),.w_en(k_w_en),.r_addr(k_read_addr_a[14-1:0]),
              .w_addr(k_write_addr[14-1:0]),.sram_data_in(data_in_k),.sram_data_out(k_sram_out_a));
    reg [14-1:0] k_read_addr_b;
    wire [DATA_WIDTH-1:0] k_sram_out_b;
    ...
    reg [DATA_WIDTH-1:0] score_write_data;
    wire [DATA_WIDTH-1:0] score_sram_out;
    sram #(.N_CHANNELS(1),.DATA_WIDTH(DATA_WIDTH),.DEPTH(2257))
    score_sram (.clk(clk),.rst(rst),.w_en(score_w_en),.r_addr(score_read_addr),
                .w_addr(score_write_addr),.sram_data_in(score_write_data),.sram_data_out(score_sram_out));
...
module softmax_approx_b1_h1 #(
    ...
    reg [DATA_WIDTH-1:0] exp_write_data;
    wire [DATA_WIDTH-1:0] exp_sram_out;
    sram #(.N_CHANNELS(1),.DATA_WIDTH(DATA_WIDTH),.DEPTH(2258))
    exp_sram (.clk(clk),.rst(rst),.w_en(exp_w_en),.r_addr(exp_read_addr),
              .w_addr(exp_write_addr),.sram_data_in(exp_write_data),.sram_data_out(exp_sram_out));
    reg [2*DATA_WIDTH-1:0] exp_sum;
    ...
    reg out_w_en;
    wire [DATA_WIDTH-1:0] out_sram_out;
    sram #(.N_CHANNELS(1),.DATA_WIDTH(DATA_WIDTH),.DEPTH(2259))
    out_sram (.clk(clk),.rst(rst),.w_en(out_w_en),.r_addr(out_read_addr),
              .w_addr(out_write_addr),.sram_data_in(norm_val),.sram_data_out(out_sram_out));
...
module dimm_weighted_sum_b1_h1 #(
    ...
    reg attn_w_en;
    wire [DATA_WIDTH-1:0] attn_sram_out;
    sram #(.N_CHANNELS(1),.DATA_WIDTH(DATA_WIDTH),.DEPTH(2260))
    attn_sram (.clk(clk),.rst(rst),.w_en(attn_w_en),.r_addr(attn_read_addr),
               .w_addr(attn_write_addr),.sram_data_in(data_in_attn),.sram_data_out(attn_sram_out));
    ...
    reg [DATA_WIDTH-1:0] log_attn_write_data;
    wire [DATA_WIDTH-1:0] log_attn_sram_out;
    sram #(.N_CHANNELS(1),.DATA_WIDTH(DATA_WIDTH),.DEPTH(2261))
    log_attn_sram (.clk(clk),.rst(rst),.w_en(log_attn_w_en),.r_addr(log_attn_read_addr),
                   .w_addr(log_attn_write_addr),.sram_data_in(log_attn_write_data),.sram_data_out(log_attn_sram_out));
