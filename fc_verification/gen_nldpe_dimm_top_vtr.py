#!/usr/bin/env python3
"""
VTR + IMC experiment for NL-DPE full DIMM top (W=16).

Reads fc_verification/rtl/nldpe_dimm_top_d64_c128.v (generated separately by
nl_dpe/gen_dimm_nldpe_top.py). Runs VTR 3 seeds, parses Fmax/CLB/BRAM/DPE/DSP,
runs IMC at VTR-measured Fmax, saves a JSON summary.

Expected metrics (Phase L targets):
  DPE count: exactly 64 (4 stages × 16 lanes)
  DSP count: ~0 (NL-DPE uses DPEs, not DSPs)
  Fmax/CLB/BRAM: accept whatever VTR reports

Usage:
    python3 fc_verification/gen_nldpe_dimm_top_vtr.py
    python3 fc_verification/gen_nldpe_dimm_top_vtr.py --skip-vtr  # IMC only
"""
import sys
import os
import json
...
RTL_PATH = PROJECT_ROOT / "fc_verification" / "rtl" / "nldpe_dimm_top_d64_c128.v"
VTR_DIR = PROJECT_ROOT / "fc_verification" / "vtr_runs" / "nldpe_dimm_top_d64_c128"
...
def regenerate_rtl():
    """Regenerate the NL-DPE full DIMM top RTL from the generator."""
    gen = PROJECT_ROOT / "nl_dpe" / "gen_dimm_nldpe_top.py"
...
def run_vtr_seeds():
    ...
    for seed in SEEDS:
        ...
        result = run_single(
            ...
            design_name="nldpe_dimm_top_d64_c128",
        )
...
def run_imc(fmax_mhz):
    """Run IMC simulator for the attention workload at given Fmax."""
    ...
    cfg['fpga_specs']['total_dimm_dpes'] = W

    patched_cfg = RESULTS_DIR / "nldpe_dimm_top_imc_config.json"
...
def main():
    ...
    print("=" * 68)
    print(f"  NL-DPE Full DIMM Top VTR + IMC (W={W}, N={N_SEQ}, d={D_HEAD}, C={C})")
    print("=" * 68)
    ...
    if not args.skip_vtr:
        ...
        avg_fmax = sum(fmax_list) / len(fmax_list) if fmax_list else 341.0
    ...
    results = {
        ...
        "wc": resources.get('wc', 0) if resources else 0,  # DPE count
    ...
    results_path = RESULTS_DIR / "nldpe_dimm_top_vtr_imc_results.json"
    with open(results_path, 'w') as f:
        json.dump(results, f, indent=2)
    print(f"  Results: {results_path}")

    if imc_result:
        with open(RESULTS_DIR / "nldpe_dimm_top_imc_output.txt", 'w') as f:
