// NL-DPE DIMM mac_sv — N=128, d=64, C=128, R=1024
// Generated by gen_dimm_nldpe_full.py

// nldpe_dimm_mac_sv
// N=128 (seq_len, input), d=64 (output), C=128 (crossbar cols)
// Pipeline: attn SRAM → DPE(VMM with V weights) → output SRAM
// Crossbar weights = V matrix (128 rows × 64 cols), preloaded
// DPE passes per invocation: 1

module nldpe_dimm_mac_sv #(
    parameter N = 128,
    parameter D = 64,
    parameter C = 128,
    ...
    // DPE with V-weights (preloaded). ACAM_MODE=0 for plain VMM.
    wire sv_dpe_dpe_done, sv_dpe_reg_full;
    wire sv_dpe_MSB_SA_Ready, sv_dpe_shift_add_done, sv_dpe_shift_add_bypass_ctrl;
    reg  [1:0] sv_dpe_nl_dpe_control;
    reg        sv_dpe_dpe_exec;
    wire [DATA_WIDTH-1:0] sv_dpe_data_in, sv_dpe_data_out;
    wire       sv_dpe_w_buf_en;

    always @(posedge clk or posedge rst) begin
        if (rst) begin sv_dpe_dpe_exec <= 0; sv_dpe_nl_dpe_control <= 0; end
        else begin
            sv_dpe_dpe_exec <= sv_dpe_reg_full;
            sv_dpe_nl_dpe_control <= sv_dpe_dpe_exec ? 2'b11 : 2'b00;
        end
    end

    assign sv_dpe_data_in = in_sram_out;
    assign sv_dpe_w_buf_en = (state == S_COMPUTE) &&
                             (mac_count >= 2) && (mac_count <= PACKED_N + 1);

    dpe sv_dpe (
        .clk(clk), .reset(rst),
        .data_in(sv_dpe_data_in),
        .nl_dpe_control(sv_dpe_nl_dpe_control),
        .shift_add_control(sv_dpe_MSB_SA_Ready),
        .w_buf_en(sv_dpe_w_buf_en),
        .shift_add_bypass(1'b0),
        .load_output_reg(sv_dpe_shift_add_done),
        .load_input_reg(1'b0),
        .MSB_SA_Ready(sv_dpe_MSB_SA_Ready),
        .data_out(sv_dpe_data_out),
        .dpe_done(sv_dpe_dpe_done),
        .reg_full(sv_dpe_reg_full),
        .shift_add_done(sv_dpe_shift_add_done),
        .shift_add_bypass_ctrl(sv_dpe_shift_add_bypass_ctrl)
    );

    // Output SRAM: d output values packed into 13 words.
    reg [4-1:0] out_w_addr, out_r_addr;
    reg out_w_en;
    ...
    always @(posedge clk or posedge rst) begin
        ...
        else if (sv_dpe_dpe_done && capture_addr < PACKED_D) begin
            out_write_data <= sv_dpe_data_out;
    ...
    always @(posedge clk or posedge rst) begin
        if (rst) begin
            ...
            case (state)
                ...
                S_WAIT_DPE: begin
                    if (dpe_output_done && sv_dpe_MSB_SA_Ready) state <= S_OUTPUT;
                end
                S_OUTPUT: begin
                    if (!ready_n) begin
                        out_r_addr <= out_r_addr + 1;
                        if (out_r_addr == PACKED_D) state <= S_IDLE;
                    end
                end
            endcase
        end
