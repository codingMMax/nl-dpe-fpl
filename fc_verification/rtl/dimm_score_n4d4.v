module dimm_score_matrix #(
    ...
    reg [3-1:0] k_read_addr_b;
    wire [DATA_WIDTH-1:0] k_sram_out_b;
    sram #(.N_CHANNELS(1),.DATA_WIDTH(DATA_WIDTH),.DEPTH(5))
    k_sram_b (.clk(clk),.rst(rst),.w_en(k_w_en),.r_addr(k_read_addr_b[3-1:0]),
              .w_addr(k_write_addr[3-1:0]),.sram_data_in(data_in_k),.sram_data_out(k_sram_out_b));
    ...
    // DPE(I|exp) stage: dual-identity crossbar + ACAM=exp (KW=8)
    ...
    // Byte-wise accumulator: sum all int8 results per DPE output word
    wire [31:0] byte_sum;  // sum of EPW int8 values in one output word
    assign byte_sum = {24'b0, dimm_exp_data_out[0*8 +: 8]} + {24'b0, dimm_exp_data_out[1*8 +: 8]} + {24'b0, dimm_exp_data_out[2*8 +: 8]} + {24'b0, dimm_exp_data_out[3*8 +: 8]} + {24'b0, dimm_exp_data_out[4*8 +: 8]};
    reg [31:0] accumulator_a, accumulator_b;
    reg acc_phase;
    always @(posedge clk) begin
        if (rst || acc_clear) begin accumulator_a <= 0; accumulator_b <= 0; end
        else if (dimm_exp_valid_n) begin
            if (!acc_phase) accumulator_a <= accumulator_a + byte_sum;
            else accumulator_b <= accumulator_b + byte_sum;
        end
    end
    reg acc_clear;
    reg dpe_output_done;  // set when all DPE outputs received
    reg [15:0] dpe_out_count;  // count valid_n pulses
    always @(posedge clk) begin
        if (rst || acc_clear) begin dpe_out_count <= 0; dpe_output_done <= 0; end
        else if (dimm_exp_valid_n) begin
            dpe_out_count <= dpe_out_count + 1;
            // DPE output done when all output cycles received
    ...
    always @(posedge clk or posedge rst) begin
        if (rst) begin
            ...
            case (state)
                ...
                S_COMPUTE: begin
                    // Feed phase: read Q/K, produce log_sum, feed to DPE
                    q_read_addr <= mac_count;
                    k_read_addr_a <= score_idx * 1 + mac_count;
                    k_read_addr_b <= (score_idx + 1) * 1 + mac_count;
                    if (!feed_half) begin
                        ...
                        if (mac_count == 0) begin
                            mac_count <= 0;
                            feed_half <= 0;
                            state <= S_WAIT_DPE;  // wait for DPE to finish
                        end
                    end
                end
                S_WAIT_DPE: begin
                    // Wait for DPE to process and produce all output
