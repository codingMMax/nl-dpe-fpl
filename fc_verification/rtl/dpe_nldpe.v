// dpe_nldpe.v -- behavioral DPE model
...
//   Transitions to S_OUTPUT on the cycle nl_dpe_control deasserts.
//   No internal compute counter; precision/pipeline depth is owned
//   by the controller (workload top / TB), which holds
...
// (or on the last LOAD strobe combinationally when ctrl is already 2'b11).
    ...
    always @(posedge clk or posedge reset) begin
        if (reset) begin
            ...
            case (state)
                ...
                S_LOAD: begin
                    if (w_buf_en) begin
                        if (strobe_count + 1 >= LOAD_STROBES) begin
                            ...
                            // combinationally when ctrl is already 2'b11.
                            for (b = 0; b < ELEMS_PER_STROBE; b = b + 1)
                ...
                S_COMPUTE: begin
                    // Model Y: stay here while controller holds nl_dpe_control = 2'b11.
                    // Transition to S_OUTPUT when controller deasserts (precision-driven).
                    if (nl_dpe_control != 2'b11) begin
                        MSB_SA_Ready <= 1;
                        shift_add_done <= 1;
                        output_col_idx <= 0;
