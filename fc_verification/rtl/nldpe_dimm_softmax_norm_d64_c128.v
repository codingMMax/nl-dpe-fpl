// NL-DPE DIMM softmax_norm — N=128, C=128
// Generated by gen_dimm_nldpe_full.py

// nldpe_dimm_softmax_norm
// N=128 (sequence length), C=128 (crossbar cols), DATA_WIDTH=40
// Pipeline: input SRAM → DPE(I|log) → CLB subtract log_sum → output SRAM
// DPE passes per invocation: 1  (N/C = 128/128)

module nldpe_dimm_softmax_norm #(
    parameter N = 128,
    parameter C = 128,
    parameter DATA_WIDTH = 40
)(
    input wire clk, input wire rst,
    input wire valid_in, input wire ready_n,
    input wire [DATA_WIDTH-1:0] data_in,
    output wire [DATA_WIDTH-1:0] data_out,
    output wire ready_in, output wire valid_n
);

    localparam EPW = DATA_WIDTH / 8;  // 5
    localparam PACKED_N = 26;   // packed input words
    localparam KW_PACKED = 26;  // packed words fed per DPE pass

    // Input SRAM: N=128 exp-values packed into 26 words
    reg [5-1:0] in_w_addr, in_r_addr;
    wire in_w_en = (state == S_LOAD) && valid_in;
    wire [DATA_WIDTH-1:0] in_sram_out;
    sram #(.N_CHANNELS(1),.DATA_WIDTH(DATA_WIDTH),.DEPTH(27))
        in_sram (.clk(clk),.rst(rst),.w_en(in_w_en),
                 .r_addr(in_r_addr),.w_addr(in_w_addr),
                 .sram_data_in(data_in),.sram_data_out(in_sram_out));

    // CLB running sum: byte-wise, lane-parallel. One 32-bit total.
    // Accumulated during feed; used for log_sum subtract after DPE output.
    reg [31:0] sum_accum;
    wire [31:0] sum_lane_total;
    assign sum_lane_total = {24'b0, in_sram_out[0*8 +: 8]} + {24'b0, in_sram_out[1*8 +: 8]} + {24'b0, in_sram_out[2*8 +: 8]} + {24'b0, in_sram_out[3*8 +: 8]} + {24'b0, in_sram_out[4*8 +: 8]};
    wire feed_active = (state == S_COMPUTE)
                       && (mac_count >= 2) && (mac_count <= PACKED_N + 1);
    always @(posedge clk or posedge rst) begin
        if (rst) begin sum_accum <= 0; end
        else if (feed_active) begin
            sum_accum <= sum_accum + sum_lane_total;
        end else if (state == S_IDLE || state == S_LOAD) begin
            sum_accum <= 0;
        end
    end

    // log_sum = log(sum_accum) via LUT (modeled: log(x) ≈ x - 1 for int8 scale)
    // Kept as 1-cycle combinational for simulator alignment; FSM waits 1 cycle.
    wire [7:0] log_sum = sum_accum[7:0] - 8'd1;

    // DPE(I|log): identity crossbar, ACAM configured for log.
    wire norm_dpe_dpe_done, norm_dpe_reg_full;
    wire norm_dpe_MSB_SA_Ready, norm_dpe_shift_add_done, norm_dpe_shift_add_bypass_ctrl;
    reg  [1:0] norm_dpe_nl_dpe_control;
    reg        norm_dpe_dpe_exec;
    wire [DATA_WIDTH-1:0] norm_dpe_data_in, norm_dpe_data_out;
    wire       norm_dpe_w_buf_en;

    always @(posedge clk or posedge rst) begin
        if (rst) begin norm_dpe_dpe_exec <= 0; norm_dpe_nl_dpe_control <= 0; end
        else begin
            norm_dpe_dpe_exec <= norm_dpe_reg_full;
            norm_dpe_nl_dpe_control <= norm_dpe_dpe_exec ? 2'b11 : 2'b00;
        end
    end

    // Feed: input SRAM out → DPE data_in. Valid when SRAM output ready (2-cyc latency).
    assign norm_dpe_data_in = in_sram_out;
    assign norm_dpe_w_buf_en = (state == S_COMPUTE) &&
                               (mac_count >= 2) && (mac_count <= PACKED_N + 1);

    dpe norm_dpe (
        .clk(clk), .reset(rst),
        .data_in(norm_dpe_data_in),
        .nl_dpe_control(norm_dpe_nl_dpe_control),
        .shift_add_control(norm_dpe_MSB_SA_Ready),
        .w_buf_en(norm_dpe_w_buf_en),
        .shift_add_bypass(1'b0),
        .load_output_reg(norm_dpe_shift_add_done),
        .load_input_reg(1'b0),
        .MSB_SA_Ready(norm_dpe_MSB_SA_Ready),
        .data_out(norm_dpe_data_out),
        .dpe_done(norm_dpe_dpe_done),
        .reg_full(norm_dpe_reg_full),
        .shift_add_done(norm_dpe_shift_add_done),
        .shift_add_bypass_ctrl(norm_dpe_shift_add_bypass_ctrl)
    );

    // Output SRAM: N normalized log values packed into 26 words.
    reg [5-1:0] out_w_addr, out_r_addr;
    reg out_w_en;
    reg [DATA_WIDTH-1:0] out_write_data;
    wire [DATA_WIDTH-1:0] out_sram_out;
    sram #(.N_CHANNELS(1),.DATA_WIDTH(DATA_WIDTH),.DEPTH(27))
        out_sram (.clk(clk),.rst(rst),.w_en(out_w_en),
                  .r_addr(out_r_addr),.w_addr(out_w_addr),
                  .sram_data_in(out_write_data),.sram_data_out(out_sram_out));

    // CLB byte-wise subtract: out_byte[b] = dpe_out[b] - log_sum
    wire [DATA_WIDTH-1:0] norm_out_word;
    genvar gn;
    generate for (gn = 0; gn < EPW; gn = gn + 1) begin : norm_sub
        assign norm_out_word[gn*8 +: 8] = norm_dpe_data_out[gn*8 +: 8] - log_sum;
    end endgenerate

    localparam S_IDLE = 3'd0, S_LOAD = 3'd1, S_COMPUTE = 3'd2,
               S_WAIT_DPE = 3'd3, S_WRITE_OUT = 3'd4, S_OUTPUT = 3'd5;

    reg [2:0] state;
    reg [5-1:0] mac_count;
    reg [5-1:0] out_write_count;
    reg dpe_output_done;
    reg [15:0] dpe_out_count;

    // Track DPE output progress
    always @(posedge clk or posedge rst) begin
        if (rst) begin dpe_out_count <= 0; dpe_output_done <= 0; end
        else if (state == S_IDLE) begin dpe_out_count <= 0; dpe_output_done <= 0; end
        else if (norm_dpe_dpe_done) begin
            dpe_out_count <= dpe_out_count + 1;
            if (dpe_out_count + 1 >= KW_PACKED) dpe_output_done <= 1;
        end
    end

    // Capture DPE output into output SRAM (with CLB log_sum subtract)
    reg [5-1:0] capture_addr;
    always @(posedge clk or posedge rst) begin
        if (rst) begin capture_addr <= 0; out_w_en <= 0; end
        else if (state == S_IDLE) begin capture_addr <= 0; out_w_en <= 0; end
        else if (norm_dpe_dpe_done) begin
            out_write_data <= norm_out_word;
            out_w_addr <= capture_addr;
            out_w_en <= 1;
            capture_addr <= capture_addr + 1;
        end else begin out_w_en <= 0; end
    end

    always @(posedge clk or posedge rst) begin
        if (rst) begin
            state <= S_IDLE;
            in_w_addr <= 0; in_r_addr <= 0;
            out_r_addr <= 0;
            mac_count <= 0; out_write_count <= 0;
        end else begin
            case (state)
                S_IDLE: if (valid_in) state <= S_LOAD;
                S_LOAD: begin
                    if (valid_in) in_w_addr <= in_w_addr + 1;
                    if (in_w_addr == PACKED_N) begin
                        state <= S_COMPUTE;
                        mac_count <= 0;
                    end
                end
                S_COMPUTE: begin
                    // Set SRAM read addr; 2-cycle SRAM latency → valid from mac=2
                    if (mac_count < PACKED_N) in_r_addr <= mac_count;
                    mac_count <= mac_count + 1;
                    if (mac_count == PACKED_N + 1) state <= S_WAIT_DPE;
                end
                S_WAIT_DPE: begin
                    if (dpe_output_done && norm_dpe_MSB_SA_Ready) begin
                        state <= S_OUTPUT;
                    end
                end
                S_OUTPUT: begin
                    // Ready for downstream; loop back to idle after read
                    if (!ready_n) begin
                        out_r_addr <= out_r_addr + 1;
                        if (out_r_addr == PACKED_N) state <= S_IDLE;
                    end
                end
            endcase
        end
